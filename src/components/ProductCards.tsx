...
export default function ProductCards() {
  return (
    <section id="products" className="bg-cream px-6 pb-10 pt-1">
      <div className="mx-auto max-w-[864px]">
        <div className="grid grid-cols-1 items-stretch gap-8 md:grid-cols-3">
          {CARDS.map((card) => (
            <div
              key={card.title}
              className="flex min-h-[276px] flex-col items-center rounded-[18px] border border-[#efe2cf] bg-white/78 px-6 py-5 text-center shadow-[0_8px_22px_rgba(83,55,24,0.12)] transition-all duration-300 hover:-translate-y-1 hover:shadow-xl"
            >
              ...
              <Link
                href={card.href}
                className="mt-5 flex min-h-6 items-center justify-center gap-2 font-body text-[14px] font-extrabold leading-tight text-orange hover:underline"
              >
                <span>{card.link}</span>
                <svg
                  viewBox="0 0 20 20"
